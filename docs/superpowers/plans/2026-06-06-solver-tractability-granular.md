...
> **REFINEMENT (post-T1 design review):** Split into **T2a** (soft 16-grid value ordering in `search.ts`) and **T2b** (add-a-term relax). The relax ladder's "allow up to ceiling/18" lever is realised by T2a's soft ordering (the search already caps at `creditCeiling`=18; the soft grid just *prefers* ≤16 and falls back to ≤18 within the SAME search — no horizon change, no retry). The "add a term" lever (T2b) is needed only when a *derived* horizon is structurally too SHORT (a prereq chain deeper than ⌈creditsNeeded/16⌉ terms — credits alone always fit, since ⌈N/16⌉×18 ≥ N). **T2b must live at the `build.ts` level, NOT inside `solveForwardSchedule`:** extending the horizon changes `graduationTerm`, and the validator's `graduationTargetTerm` is baked into `validatorRules` built in `build.ts` from the original term — a relax inside the solver would extend the internal horizon but the post-hoc `runGraduationPathValidator` would still check the original target and (wrongly) report the relaxed plan infeasible. So T2b re-derives `graduationTerm` → rebuilds `validatorRules` via `buildSolverInputWithRules` → re-solves+re-validates, keeping everything consistent (the same pattern `simulateAlternatives`'s `extend_grad_one_term` already uses). A student-stated *hard* target is never extended (binding constraints reported — PLAN-13).
