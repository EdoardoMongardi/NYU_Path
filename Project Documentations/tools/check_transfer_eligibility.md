...
> **Today there is exactly one authored route file: `cas_to_stern.json`.** Every other `<from>_to_<to>` combination hits this `unsupported` branch. `check_transfer_eligibility` itself has no fallback to RAG or the program corpus — transfer eligibility is intentionally JSON-table-driven, not retrieval-driven. **However (improvement plan, Phase C):** `internal-transfer-equivalencies/` is now ingested into the policy corpus (school `all`, `category:admissions`), so the *agent* can still surface internal-transfer requirements for any school via `search_policy` / `get_program_requirements` once the corpus is regenerated — it just won't get a structured eligibility verdict from this tool. The structured `unsupported` branch and the RAG path are complementary: one gives a deterministic verdict (Stern only today), the other gives cited-but-estimated requirement text for the rest.
