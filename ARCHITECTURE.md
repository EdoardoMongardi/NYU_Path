...
### Phase -1: De-Hardcode v1 Engine (3 days)

> **Goal:** Remove all CS-BA-specific hardcoding from the generic engine so it works for any major. This is the prerequisite for everything else.

```
MODIFY: packages/engine/src/audit/degreeAudit.ts
  - DELETE: lines 109-119 (csciCreditsCompleted counter)
  - DELETE: lines 237-240 (isCSProgram heuristic check)
  - REPLACE: The CSCI-UA 32-credit minimum becomes a `min_credits` rule
    in the program JSON (cas_cs_ba.json), evaluated by the generic
    ruleEvaluator like every other rule. No special-case code.
  Total: ~15 lines removed, 0 lines added to this file.

MODIFY: packages/engine/src/audit/creditCapValidator.ts
  - DELETE: checkCSCICredits() function (lines 206-235)
  - DELETE: the isCSMajor option and its callsite (lines 76-83)
  - KEEP: All 6 universal credit cap checks (residency, non-home-school, online,
    transfer, advanced standing, P/F) — these apply to ALL students, but
    thresholds will be read from SchoolConfig instead of hardcoded.
  - REPLACE: Per-major credit minimums become rules in program JSON,
    evaluated by ruleEvaluator. creditCapValidator only does school-wide caps.
  Total: ~30 lines removed.

MODIFY: packages/engine/src/planner/priorityScorer.ts
  - DELETE: isCorePrereq() hardcoded function (lines 163-173)
  - REPLACE WITH: Dynamic lookup from program's `must_take` rules.
    Any course in a `must_take` rule that is also a prereq for other
    courses gets the CORE_PREREQ boost. Works for any major.
  Total: ~10 lines changed.

MODIFY: packages/engine/src/planner/semesterPlanner.ts
  - CHANGE: planNextSemester() currently takes a single `program`.
    Update to accept `programs: Program[]` and merge audit results
    across all declared programs before scoring.
  Total: ~20 lines changed.

DEPRECATE: Move replaced files to _deprecated/
  mv packages/engine/src/chat/ → _deprecated/chat/
  mv packages/engine/src/data/academicRules.ts → _deprecated/
  mv packages/engine/src/search/semanticSearch.ts → _deprecated/

Total: ~75 lines removed/changed across 4 files. 0 new files.
After this phase, the engine is major-agnostic.
```

### Phase 0: Missing Deterministic Checks (1 week)

```
MODIFY: packages/engine/src/audit/creditCapValidator.ts
  + Non-home-school credit cap (G29) — refactor to read from SchoolConfig.creditCaps
  + School residency credits check (G13) — refactor to read from SchoolConfig.residency
  + Advanced standing cap (G31) — already exists, verify logic

MODIFY: packages/engine/src/audit/degreeAudit.ts
  + Handle I/NR/W grades as non-earned (G32-G34)
  + Verify grade thresholds from SchoolConfig.gradeThresholds (G35)

MODIFY: packages/engine/src/audit/academicStanding.ts
  + Fix SAP formula: attempted includes W, NR, F (G34)

MODIFY: packages/engine/src/audit/gpaCalculator.ts (if not yet created)
  + Per-major GPA calculation (G5-G6)

Total new code: ~50 lines across 4 existing files
```

### Phase 1: Data Scaling + File Deprecation (2-3 weeks)

> **Updated:** Expanded scope to incorporate all findings from the bulletin data gap audit (14 issues across 15 schools).

```
MODIFY: packages/shared/src/types.ts (FIRST PASS — StudentProfile + Course)
  + Add: homeSchool: string (REQUIRED — standalone field, not derived from programs)
  + Add: isExternalTransfer?: boolean
  + Add: visaStatus, targetGraduationTerm, foreignLanguageExempt, catalogYear,
    workloadPreference, preferences, standingLevel to StudentProfile
  + Add: completedMilestones? to StudentProfile
  + Add: isOnline, courseLevel, courseSuffix, courseFlags? to Course
  + Add: pfCoursesElected: number  ← track P/F elections against career limit
  + Add: pfCoursesThisTerm: number ← track per-term/per-year P/F limit
  + Add: liberalStudiesTransition?: {  ← Issue #10
      targetSchool: string,
      semestersCompleted: number,
      transitionDeadlineTerm: string,
      concentrationPlanApproved: boolean
    }
  + Add: StandingLevel type = "freshman" | "sophomore" | "junior" | "senior"
  + Add: ProgramType = "major" | "minor" | "concentration"  ← Issue #11: Stern concentrations
  + CHANGE: declaredPrograms from ProgramInfo[] to ProgramDeclaration[]
  + Add: ProgramDeclaration interface:
    - programId: string
    - name: string
    - type: ProgramType          ← "major" | "minor" | "concentration"
    - status: "declared" | "exploring" | "intended"
    - declaredDate?: string
    (replaces previous simple program object — "exploring" = considering,
     "intended" = planning to declare/transfer, "declared" = official)
  + Add: InternalTransferRequirements interface:
    - sourceSchoolId, targetSchoolId: string
    - applicationDeadline: string (e.g., "march_1")
    - acceptedTerms: ("fall" | "spring")[]
    - minCreditsCompleted: number (typically 32)
    - maxCreditsForEligibility?: number
    - disqualifiers?: string[] (e.g., "previously_external_transfer")
    - entryYearRequirements: EntryYearReqs[] (prereqs differ by soph vs junior)
    - notes?: string[], equivalencyUrl?: string
  + Add: EntryYearReqs interface:
    - entryYear: "sophomore" | "junior"
    - requiredCourseCategories: { category, description, satisfiedBy: string[], minCount }[]

MODIFY: packages/shared/src/types.ts (SECOND PASS — SchoolConfig)
  + Add: SchoolConfig interface:
    - residency: { type: "suffix_based" | "total_nyu_credits", suffix?, minCredits, ... }
    - creditCaps, gradeThresholds
    - courseSuffix: string[] (ARRAY, not single string — SPS has ["-UC", "-CE"])
    - sharedPrograms, timeLimitYears
    - auditMode?: "standard" | "advising_only" | "phased" | "informational"
    - programExclusions?: ProgramExclusion[]
    - passFail: PassFailConfig          ← Issue #1: per-school P/F rules
    - spsPolicy: SpsPolicy              ← Issue #2: SPS enrollment blockers
    - doubleCounting: DoubleCountConfig  ← Issue #3: per-school overlap rules
    - transferCreditLimits: TransferLimits  ← Issue #4: NYU-wide + per-school
    - acceptsTransferCredit: boolean    ← Issue #5: eligibility flag
    - lifecycle?: LifecycleConfig       ← Issue #10: forced-exit programs
    - supportedProgramTypes?: ProgramType[]  ← Issue #11: concentration support
    - deansListThreshold?: { minGpa, minCredits, per }
    - overloadRequirements: OverloadCondition[]  ← Issue #8: tiered conditions
  + Add: ProgramExclusion interface:
    - if: string[] (program IDs), thenCannotDeclare: string, reason: string
  + Add: PassFailConfig interface:
    - careerLimitType, careerLimit, perTermLimit, perTermUnit,
      countsForMajor, countsForMinor, canElect, autoExcludedFromLimit,
      excludedCourseTypes, gradePassEquivalent, failCountsInGpa
  + Add: SpsPolicy interface:
    - allowed, allowedPrefixes?, creditType?, countsTowardResidency?
  + Add: DoubleCountConfig interface:
    - defaultMajorToMajor, defaultMajorToMinor, defaultMinorToMinor?,
      noTripleCounting, requiresDepartmentApproval, overrideByProgram?
  + Add: TransferLimits interface:
    - firstYearMaxTotal, transferStudentMaxTotal, springAdmitPostSecondaryMax?
  + Add: LifecycleConfig interface:
    - type, expectedTransitionSemesters, maxSemesters, transitionTarget,
      transitionRequires, warningThreshold, dualAuditMode
  + Add: OverloadCondition interface:
    - condition, minGpa?, minCreditsCompleted?, minSemesters?, maxCredits?

MODIFY: packages/shared/src/types.ts (THIRD PASS — Rule + Program extensions)
  + Add to Rule interface:
    - alternatives?: Record<string, string[]> (or-course patterns, par11.3e)
    - minStanding?: StandingLevel (standing gates, par11.3f)
    - poolConstraints?: PoolConstraint[] (cross-school pools, par11.3g)
    - creditRange?: [number, number] (display-only, par11.3h)
    - courseFilter?: CourseFilter (suffix-based filtering, par11.3j)  ← Issue #7
    - minCreditsPerCourse?: number (Tandon "each at least 3 credits")
    - substitutions?, sequenced?, excludePool?, excludeFlags? (already planned)
  + Add: CourseFilter interface:
    - type: "suffix_allowlist"
    - allowedSuffixes: string[], excludedSuffixes?: string[]
  + Add: PoolConstraint interface:
    - pool: string[], minCredits: number, label: string
  + Add: SubstitutionCondition type (already planned)
  + Add to Program interface:
    - degreeLevel: "bachelor" | "associate" | "combined" | "accelerated"
    - auditMode?: "standard" | "phased" | "informational"
    - totalCreditsRequired: number (per-program, not just per-school)
    - maxSharedCourses?: number (default 2, Steinhardt depts may be 0 or 1)
    - phaseGates?: PhaseGate[]
    - trackDeclarationDeadline?, trackRequirementsMode? ("additive" | "alternative")
    - majorResidency?: MajorResidency  ← per-program study abroad limits (CS: ≥50% -UA)
    - tracks[]?.gpaGate?: { overall, inMajor }  ← honors entry GPA threshold
    - tracks[]?.admissionDeadline?: string  ← when to apply for honors
    - tracks[]?.additionalCredits?: number  ← how many extra credits honors adds
  + Add: PhaseGate interface:
    - phaseId, label, afterYear, requiredGpa, requiredGpaScope,
      minGradeInMajorCourses?, advancesTo, failureAction?
  + Add: MajorResidency interface:
    - minPercent: number (e.g. 50)
    - countingSuffixes: string[] (e.g. ["CSCI-UA", "MATH-UA"])
    - excludedSuffixes?: string[] (e.g. ["CS-UH", "MATH-UH", "CSCI-SHU"])
    - scope: "major_courses" | "major_and_minor_courses"
  + Add: milestone rule type with milestoneType, hoursRequired,
    appliesFromCatalogYear?, appliesFromClass?

MODIFY: packages/engine/src/dataLoader.ts
  + Multi-file loading from programs/ and schools/ directories
  + School config loader: loadSchoolConfig(schoolId) -> SchoolConfig
  + Residency checker: handles both suffix_based and total_nyu_credits modes
  + Fallback: if programId or schoolId not found -> return { status: "unsupported" }

MODIFY: packages/engine/src/audit/ruleEvaluator.ts
  + ~10 lines: alternatives check in must_take evaluation (par11.3e)
  + ~5 lines: minStanding gate before rule evaluation (par11.3f)
  + ~15 lines: poolConstraints validation in choose_n (par11.3g)
  + ~5 lines: minCreditsPerCourse filtering in choose_n (par11.3h)
  + ~10 lines: courseFilter suffix_allowlist support in min_credits (par11.3j Issue #7)
  + ~30 lines: substitution check before standard evaluation (par4.5)
  + ~30 lines: milestone rule type evaluation
  + ~10 lines: excludePool filtering (already planned)
  + ~10 lines: track rules merging with additive vs alternative modes
  + ~15 lines: majorResidency check — count courses by prefix, flag if <minPercent% (par11.3c)
  + ~10 lines: honors track gpaGate validation — emit track_gpa_risk if below threshold

MODIFY: packages/engine/src/audit/creditCapValidator.ts
  + Refactor to read SchoolConfig instead of hardcoded school-specific caps
  + Support flexible residency: suffix_based (count -UA credits) vs
    total_nyu_credits (count any NYU credits regardless of suffix)
  + Support all cap types: non_home_school, online, transfer,
    advanced_standing, specific_school, category
  + Add: transferCreditLimits enforcement (Issue #4):
    - firstYearMaxTotal: 32 (NYU-wide) vs transferStudentMaxTotal per school
    - springAdmitPostSecondaryMax: 8 (CAS)
  + ~50 lines changed (replace hardcoded values with config-driven)

CREATE: packages/engine/src/audit/passfailGuard.ts (REFACTOR — Issue #1)
  + Read SchoolConfig.passFail config for ALL decisions
  + Handle careerLimitType: "credits" | "courses" | "percent_of_program"
  + Handle perTermUnit: "semester" | "academic_year" (Stern is yearly)
  + Handle canElect: false (Tandon blocks elective P/F entirely)
  + Handle countsForMajor/countsForMinor/countsForGenEd per school
  + Handle autoExcludedFromLimit (Stern: IBEX, CLP auto-P/F don't count)
  + Handle excludedCourseTypes (Stern: no grad courses; Nursing: no clinical)
  + ~80 lines (substantially more complex than original single-school guard)

CREATE: packages/engine/src/audit/spsEnrollmentGuard.ts (NEW — Issue #2)
  + Read SchoolConfig.spsPolicy
  + If !allowed → return { blocked: true, reason: "Your school does not grant credit for SPS courses" }
  + If allowed with allowedPrefixes → check course prefix against allowlist
  + If creditType === "elective_only" → flag if course would be used for requirement
  + If countsAgainstNonHomeSchoolCap → route to non-home-school counter
  + ~30 lines
  + Called by planner before suggesting any -UC/-CE course
  + Called by search_courses when filtering recommendations

CREATE: packages/engine/src/audit/crossProgramAudit.ts
  + fullDegreeAudit() coordinator
  + STEP 0: Validate program declarations against programExclusions
  + Groups programs by school, loads school configs
  + Checks school auditMode (standard / advising_only / phased)
  + Runs degreeAudit() per program with correct school config
  + Collects substitutionApplied entries -> adds replacement rules
  + Cross-school credit cap enforcement
  + Double-counting: uses SchoolConfig.doubleCounting per school (Issue #3):
    - For cross-school programs: min(schoolA.doubleCounting, schoolB.doubleCounting)
    - For Tisch: separate defaultMajorToMajor (2) vs defaultMajorToMinor (1)
    - For Stern: default 0 (generally prohibited)
    - When doubleCounting.requiresDepartmentApproval → flag for manual review
  + Phase gate evaluation for combined/accelerated programs
  + Lifecycle handling (Issue #10): if homeSchool === "liberal_studies"
    → run dual-audit (LS core + transition school preview)

CREATE: data/schools/ directory with school config files
  + cas.json, tandon.json, stern.json, tisch.json,
    steinhardt.json, gallatin.json, silver.json, sps.json,
    nursing.json, dentistry.json, gph.json, liberal_studies.json
  + Each with correct residency model (suffix_based vs total_nyu_credits)
  + Each with school-specific programExclusions
  + Each with passFail, spsPolicy, doubleCounting, transferCreditLimits (Issues #1-5)
  + Each with overloadRequirements array (Issue #8)
  + Each with advisingContact for fallback messages

CREATE: data/programs/ directory organized by school
  + cas/cas_cs_ba.json (with CSCI min_credits from Phase -1)
  + cas/cas_core.json
  + cas/cas_econ_ba.json, cas/cas_math_ba.json, cas/cas_psych_ba.json
  + stern/stern_business_bs.json (with alternatives, minStanding, tracks)
  + sps/sps_business_aas.json (associate degree, 60 credits)
  + Combined/accelerated programs flagged as auditMode: "informational" for v1

CREATE: data/transfers/ directory with internal transfer requirement files
  + cas_to_stern.json: 32cr min, soph reqs (calculus, writing),
    junior reqs (+stats, accounting, microecon), March 1 deadline,
    disqualifier: previously_external_transfer
  + cas_to_tandon.json, cas_to_tisch.json, ls_to_cas.json, ls_to_stern.json
  + _template.json for new pairs (schema reference)
  + Source: scraped bulletin data from admissions/ pages per school

MODIFY: packages/engine/src/audit/degreeAudit.ts
  + Add: "school_only" audit mode — when student is undeclared:
    - Skip all major-specific rule evaluation
    - Run only: Core/shared requirement progress (cas_core.json),
      credit cap checks, residency tracking, GPA computation
    - Return auditMode: "school_only" in result so agent knows to explain
  + Add: "exploratory" audit mode — when all programs are status: "exploring":
    - Run each exploring program's rules with satisfied/remaining status
    - Mark results as informational (not obligations)
    - Identify "efficient" courses that satisfy requirements in multiple
      exploring programs simultaneously

CREATE: packages/engine/src/tools/checkTransferEligibility.ts
  + Loads InternalTransferRequirements from data/transfers/{source}_to_{target}.json
  + Checks: credit minimum, disqualifiers, prerequisite courses by entry year
  + Returns: eligible/not-yet-eligible, missing prereqs, deadline, caveats
  + Always caveats: "GPA requirements not published — contact admissions"
  + ~80 lines

CREATE: packages/engine/src/tools/whatIfAudit.ts
  + Runs degreeAudit with hypothetical ProgramDeclaration[] (read-only)
  + Compares with current declared programs if requested
  + Returns: courses that transfer, additional courses needed, grad impact
  + ~60 lines

MODIFY: packages/engine/src/planner/semesterPlanner.ts
  + Add: exploratory planning mode (when programs have status: "exploring")
    - Prioritize Core requirements (always needed regardless of major)
    - Include intro/prereq courses for each exploring major
    - Highlight "efficient" courses that count for multiple exploring majors
  + Add: transfer-prep planning (when programs have status: "intended")
    - Load transfer requirements for intended school
    - Prioritize missing transfer prereqs alongside Core
    - Warn about application deadlines

VERIFY: Run existing eval suite against cas_cs_ba.json to confirm
  Phase -1 refactor produces identical audit results.
```

### Phase 2: RAG Pipeline (2 weeks)

```
CREATE: packages/engine/src/search/policySearch.ts
...
CREATE: packages/engine/src/data/policyChunks/
  + Chunked policy documents with metadata
  + General CAS rules (~80 chunks)
  + CAS Core (~40 chunks)
  + Transfer policies (~30 chunks)

CREATE: packages/engine/src/search/policyEmbeddings.ts
  + Offline embedding generation script
  + Embedding index persistence (JSON or FAISS)
```

### Phase 3: Agent Orchestrator (2-3 weeks)

> **📖 Pre-implementation reading list (Claude Code source):**
> 1. Read `query.ts` L219-280 + L1715-1728 first — understand the State type and loop continuation
> 2. Read `Tool.ts` L362-523 + L703-792 — understand Tool interface and buildTool()
> 3. Read `services/tools/toolOrchestration.ts` L19-82 — understand batch partitioning
> 4. Read `query/deps.ts` — understand dependency injection for testability
> 5. Read `query/stopHooks.ts` L1-100 — understand post-response validation hooks
> 6. Read `tools/AgentTool/prompt.ts` L99-113 — understand self-contained prompt writing
...
CREATE: packages/engine/src/agent/agentOrchestrator.ts
  ...
  + Hardening check: if tool.outputMode === 'template', bypass LLM synthesis (§2.4)

CREATE: packages/engine/src/agent/toolRegistry.ts
  + buildTool() factory with safe defaults
  + 📖 Model after: Tool.ts L757-792 (TOOL_DEFAULTS + buildTool)
  + All 8 tool definitions with validateInput() and prompt()
...
CREATE: packages/engine/src/agent/responseValidator.ts
  ...
  + Part 4c: Completeness checker (did the response miss profile-relevant caveats?)
  + 📖 Model after: query/stopHooks.ts (post-response blocking errors)
  + 📖 Re-prompt pattern from: query.ts L1282-1306 (blockingErrors → state → continue)
  + Re-prompt logic (max 2 retries)
  + Hard fallback template

CREATE: packages/engine/src/agent/inputValidator.ts
  + Profile completeness checks per tool

CREATE: packages/engine/src/agent/fallbackLogger.ts
  + Structured JSONL logging for monitoring
  + Logs: fallback events, validation interventions, completeness blocks
...
CREATE: packages/engine/src/agent/templateFormatter.ts
  ...
  + F-1 compliance, credit counts, audit summaries, SAP standing
...
### Phase 4: Extended Planner + Uncertainty Pipeline (2 weeks)

```
MODIFY: packages/shared/src/types.ts
  + PlanUncertainty interface
  + PlanResult with uncertainties[] and missingDataCaveats[]

MODIFY: packages/engine/src/planner/semesterPlanner.ts
  + detectUncertainties() heuristics
  + Integration with FOSE API for real availability

MODIFY: packages/engine/src/planner/priorityScorer.ts
  + Cross-program bonus scoring
  + Difficulty and interest scoring

CREATE: packages/engine/src/planner/multiSemesterPlanner.ts
  + Iterative semester-by-semester projection

MODIFY: packages/engine/src/planner/graduationRisk.ts
  + SAP risk with corrected formula
  + 8-year time limit warning
  + Dismissal risk threshold
```

### Phase 5: Scale to All Majors (ongoing, 2-4 weeks per batch)

```
For each batch of 10 majors:
  1. Scrape NYU Bulletin for requirements
  2. Distill into per-major JSON
  3. Add department bulletin to RAG corpus
  4. Run eval suite
  5. Deploy

Target: 60+ programs across CAS
Timeline: ~6 sprints (3 months) for full coverage
```
