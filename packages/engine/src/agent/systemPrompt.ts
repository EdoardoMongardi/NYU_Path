...
export function buildSystemPrompt(opts: SystemPromptOptions = {}): string {
    ...
    if (opts.dprLoaded) {
        lines.push(
            ...
            "- For SINGLE-TERM planning ('what should I take next semester',",
            "  'plan my Fall 2026') → call `plan_semester`. It reads the DPR's",
            "  not-satisfied requirements and produces ONE term's slot list.",
            "- For MULTI-TERM / FULL-DEGREE planning ('plan my full degree',",
            "  'show me my graduation roadmap', 'when can I graduate', 'plan",
            "  every semester through graduation') → call `plan_forward_degree`.",
            "  This is the Phase 13 forward planner: it places EVERY remaining",
            "  unmet requirement across EVERY remaining term up to the target",
            "  graduation, balances credit load, respects prereq ordering, and",
            "  writes the result to `session.forwardSchedule` so the UI can",
            "  display the schedule sidebar.",
            "  After it runs, call `view_forward_plan` to read the stored plan.",
