...
function describeImpacts(field, before, after) {
    const impacts = [];
    switch (field) {
        case "homeSchool":
            impacts.push(`Audit + planner will switch to the ${after} SchoolConfig (residency, P/F, credit caps).`, `Internal-transfer eligibility checks become unsupported until the new home-school's data is loaded.`);
            break;
        case "catalogYear":
            impacts.push(`Program rules will be loaded from the ${after} snapshot (per-§11.0.3 catalog-year pinning).`, `Already-completed courses are unaffected; only future audits use the new year.`);
            break;
        case "declaredPrograms":
            impacts.push(`Audits will run against the new program list. Existing courses will be re-counted.`, `Cross-program double-counting limits re-evaluate immediately.`);
            break;
        case "visaStatus":
            impacts.push(`Enrollment checks will ${after === "f1" ? "REQUIRE F-1 full-time minimums" : "no longer apply F-1 minimums"}.`, `Plan suggestions will be re-filtered for visa-specific rules.`);
            break;
    }
...
export const updateProfileTool = buildTool({
    name: "update_profile",
    description: "Stages a preview of a single profile-field change (homeSchool / " +
    ...
    async validateInput(_input, { session }) {
        if (!session.student)
            return { ok: false, userMessage: "No student profile loaded." };
        return { ok: true };
    },
    prompt: () => `Stage a profile-update PREVIEW. The user MUST confirm before the ` +
    ...
    async call(input, { session }) {
        ...
        switch (input.field) {
            ...
            case "declaredPrograms":
                ...
                after.declaredPrograms = input.value;
                break;
            case "visaStatus":
        ...
        const mutation = {
            id,
            field: input.field,
            before: before[input.field],
            after: after[input.field],
            impacts: describeImpacts(input.field, before[input.field], after[input.field]),
        };
        if (!session.pendingMutations)
            session.pendingMutations = new Map();
...
export const confirmProfileUpdateTool = buildTool({
    ...
    description: "Applies a previously-staged profile mutation by id. ONLY call after " +
    ...
    async validateInput(input, { session }) {
        ...
        if (!pending) {
            return {
                ok: false,
                userMessage: `No pending mutation with id "${input.pendingMutationId}". ` +
                    `Either it was already consumed or update_profile was not called first.`,
            };
        }
        return { ok: true };
    },
    prompt: () => `Apply a previously-staged profile mutation. Required input: ` +
    ...
    async call(input, { session }) {
        ...
        switch (mutation.field) {
            case "homeSchool":
                student.homeSchool = mutation.after;
                break;
            case "catalogYear":
                student.catalogYear = mutation.after;
                break;
            case "declaredPrograms":
                student.declaredPrograms = mutation.after;
                break;
            case "visaStatus":
                student.visaStatus = mutation.after;
                break;
        }
