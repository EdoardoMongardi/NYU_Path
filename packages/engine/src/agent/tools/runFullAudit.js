...
export const runFullAuditTool = buildTool({
    name: "run_full_audit",
    description: "Runs a deterministic degree audit against the student's declared programs. " +
    ...
    async validateInput(_input, { session }) {
        if (!session.student)
            return { ok: false, userMessage: "I need your transcript / profile loaded before I can run an audit." };
    ...
    prompt: () => `Run a degree audit. Returns rules, courses-satisfying, remaining counts, ` +
...
//# sourceMappingURL=runFullAudit.js.map