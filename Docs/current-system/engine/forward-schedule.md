...
- **Non-CAS DPR validation: the classifier is still CAS-coupled; a synthetic fixture now PINS the seam end-to-end (it does NOT fix it).** `classifyRequirementKind` still keys its school-core / general-elective detection on CAS-specific structural rgId families (`RG5004`, `RG5007`, `RG5393`, `RG33308`, `RG5002`, `RG5005`, `RG31394`, `RG31395`) and the `R1142/*` major rId family; non-CAS hierarchies fall to its fallback paths (declared-program title match for the major group, else `unknown`). A **synthetic, clearly-labeled** non-CAS DPR (NYU Shanghai / CS, with deliberately non-CAS rgId/rId values) now runs through the full `classify → solve → validate` pipeline as a regression pin — `packages/engine/tests/e2e/nonCasPlan.test.ts` + `packages/engine/tests/fixtures/dpr_nonCas_synthetic.ts`. It asserts the pipeline does NOT silently CAS-crash: the classifier returns a `Map` over the fallback paths, and `buildForwardSchedule` returns a recognized `state` — EITHER a valid schedule OR an **honest `infeasible-draft`** carrying a populated failing-axis / `infeasibilityReport` (an explained infeasible-draft is an accepted PASS). `parseDpr` is intentionally skipped — there is no real non-CAS DPR *text* to parse, and the project philosophy (`Docs/core_philosophy.md` #2) forbids fabricating unverifiable "real" data — so the pin starts from a constructed (Zod-validated) DPR object. **REAL non-CAS DPR validation still remains PENDING a real non-CAS fixture**; this test documents the CAS-coupling seam, it does not de-CAS the classifier.
  - **Offerings for session-only courses: FIXED** (was previously folded into this pin's infeasibility story). `SolverInput.offerings` / `offeringConfidence` are now gap-filled from `session.courses[i].termsOffered` for ids the global cache lacks (see §2.1), so the synthetic `-SHU` courses DO carry offering rows. The non-CAS pin nonetheless still hits its honest `infeasible-draft` branch — but for a **distinct** reason: a downstream constraint-search / `materializePlan` binding gate emits a placeholder for the major-required leaf even though its course is now catalog- AND offering-present with its prereq satisfied. That residual binding gate is a separate, still-open limitation; the offerings gap itself is closed.
