...
> **Proactive sibling (D7.1 + D7.2).** A second, *proactive* deterministic detector — `detectElicitationOpportunity` in [`proactiveElicitation.ts`](../../../packages/engine/src/agent/proactiveElicitation.ts) (see [CORE RULE 13 in system-prompt.md](system-prompt.md)) — handles the opposite case: a **clear but context-thin** planning question, where a professional adviser would answer first and then append ONE focused follow-up. It deliberately **suppresses overlap** with `detectAmbiguity` (the reactive clarifier owns ambiguous messages) and is **append-not-substitute** — never streamed in place of the answer the way this reactive gate is.
>
> **D7.2 — route wiring (append, not substitute).** The same module exports the pure decision pair the route consumes AFTER the agent loop: `decideElicitationAppend({ report, finalText, priorMessages })` and `elicitationQuestionFor(topic)`, plus the stable `ELICITATION_LEAD_IN` marker. On the **OK terminal path only** (`allOk`), the v2 route appends `decideElicitationAppend(...).text` to `finalTextOut` *before* the `done` write, so the bounded question rides into `done.finalText` AND the durable transcript. The decision applies two guards: it stays silent when the agent's own reply already asks (ends with `?` or already carries the lead-in — *agent-already-elicited*), and it is **bounded-frequency** — it greps the LAST assistant message in `priorMessages` for `ELICITATION_LEAD_IN` and suppresses when we elicited on the immediately-prior turn (no two proactive asks back-to-back). The append-not-substitute guarantee is **structural**: `decideElicitationAppend` only ever returns text to APPEND (or `null`), never a replacement; `append:false` always pairs with `text:null`. See [chat-route-sse.md §13](../web/chat-route-sse.md) for the route seam.
