...
> **Banner corrected (D4.3, 2026-06-11; count bumped to 12 in D6.5, then to 13 in D7.1).** The file's top-of-module banner (`systemPrompt.ts:1-28`) previously called this an "Appendix A (25 rules verbatim)" prompt. It now honestly describes the current spine: **15** numbered CORE RULES, listed by name in the banner (13 through D7.1, +14 DPR-field authority / +15 IP-change verification grounding in the Phase-4 follow-ups). The banner count is load-bearing — `systemPrompt.test.ts` derives the actual number of `N. <text>` lines in the CORE RULES block and asserts the banner agrees, so a future added/removed rule that doesn't update the banner FAILS that test. There is no 25-rule list in the emitted prompt; read the function body, not training-data memory of the old Appendix A.
...
1. **Core static rules** — fifteen numbered rules that are always present
...
## 1. The fifteen core static rules (always present)

These appear verbatim under the `CORE RULES (mandatory — non-negotiable):` header. Rules 9–11 are the **D4 honesty-rail** additions (2026-06-11); rule 12 is the **D6.5 honesty interlock** addition; rule 13 is the **D7.1 proactive-elicitation** addition; rules 14 (**DPR-field authority**) and 15 (**IP-change verification grounding**) are the **Phase-4 follow-up** additions (F2 / F3, 2026-06-16). The banner count is load-bearing — `systemPrompt.test.ts` derives the `N. <text>` line count + asserts the banner agrees (now **15**).
...
15. **IP-change verification grounding (F3, 2026-06-16).** A CLAIMED change to a CURRENT-term in-progress course (drop / withdraw / pass-fail) is an UNVERIFIED assumption — the agent plans around it ONLY as a clearly-labeled draft/what-if, never folds it into the recorded plan, never tool-records it as done; only a new DPR confirms a status change. It surfaces the real registration window (the engine's `classifyIpChangeability` over the academic-calendar config — [dpr.md](dpr.md)) + the W/pass-fail consequences (a "W" does NOT fulfill the requirement; pass/fail may not satisfy a letter-grade major rule) and closes with "verify with your adviser; nothing is official until it shows on your next DPR." A FUTURE / pre-registered IP course is freely changeable. (DEFERRED, own task: the precise W/PF → requirement-satisfaction engine modeling; until then rule 15 hedges that impact in prose rather than computing it — the solver/validator are untouched.)
