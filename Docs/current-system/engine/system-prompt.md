...
> Last verified against code: 2026-06-19 (plan 37 — **8-axis validator** (was 7) now referenced in §1 rule 16 / §8 diagram / §4; **FOSE availability ≠ requirement satisfaction** guardrail (K2) added to the DPR-loaded routing block; **optional-term valid even if satisfies no requirement** (L2) added to the DPR-loaded routing block; **`get_credit_caps` output MUST be quoted verbatim** (L2) added; **after `propose_plan_change` the agent narrates the canvas Confirm button — does NOT auto-call `confirm_plan_change`** (I3) added to the preference-extraction rules; **invalid changes → agent explains WHY + what-would-make-it-valid + adviser; NEVER "apply/override anyway"** (M2) added to the preference-extraction rules; **`confirm_plan_change` inputSchema is `{ mutations }` only — no `pendingMutationId` parameter** (I3) clarified in the DPR-loaded routing block). 2026-06-18 (plan 35 — **CORE RULE 15** reworded (§6: a claimed current-term change MAY drive a CONFIRMABLE labeled what-if plan but never a DPR fact; W universal, P/F school-specific) + new **CORE RULE 16** what-if router; banner count 15→16). 2026-06-16 (Phase-4 follow-ups F2/F3 — **CORE RULE 14** DPR-field authority (read-only / change-via-re-upload) + **CORE RULE 15** IP-change verification grounding; banner count 13→15). 2026-06-13 (doc-sync pass: removed the present-tense `PlanChangeProposal` reference — that interface was deleted in cleanup PR #47). 2026-06-11 (D6.6 — the 5-bucket preference-extraction eval (A/B/C/Dpos/Dneg, ≥10 fixtures each, 59 total) now enforces its **≥85%-per-bucket bar as an operator-gated vitest assertion** (`expect(b.accuracy).toBeGreaterThanOrEqual(0.85)` per bucket, gated on `ANTHROPIC_API_KEY`, 600 s timeout) — a regression below 85% on any bucket fails CI-with-key; the unconditional invariant tests pin structure in keyless CI. The former import-side-effect that only logged the bar (and could `process.exit(1)` during collection) is now guarded by `ANTHROPIC_API_KEY && !VITEST`, so it fires only for the standalone `tsx` run. D6.5 — CORE RULE 12 (honesty interlock — recorded-not-enforced): when a scheduling preference (`setSchedulingPreference`) or a soft objective (`addSoftObjective`) is RECORDED, the agent must NOT claim the visible course-level plan changed (no "I've made Tuesday free"); it states the preference is recorded and names WHEN/HOW it applies — scheduling prefs at section materialization (strict ⇒ eliminate conflicting sections, soft ⇒ deprioritize), soft objectives as ranking bias among equally-valid plans that never changes validity. Banner/count 11→12; `renderSetSchedulingPreference` tightened to the recorded-not-enforced framing. D6.4 — Tier-D 3-layer SOFT-only guard documented as intact (Layer 1 prompt / Layer 2 compile-guard ×2 / Layer 3 Dneg eval) and rung-4 → rung-2 enablement: a genuinely-new SOFT factor may now map via `HEURISTIC_MAPPING` to `addSoftObjective` (recorded, ranker-read) instead of `null`, SOFT-only invariant preserved end to end; D6.2 — rung-1 table now carries the rung-2 `addSoftObjective`/`clearSoftObjectives` SOFT-only mapping, recorded + ranker-read, never changes validity; D6.1 — rung-1 preference table + Tier-A eval reconciled to the live `PlanMutation` union, incl. Phase-17 `move`/`unpin`/`freeze`; D4 honesty-rail CORE RULES 9–11 + banner count fix; post planning-engine rebuild, PRs #35-#41).
...
> **Banner corrected (D4.3, 2026-06-11; count bumped to 12 in D6.5, then to 13 in D7.1).** The file's top-of-module banner (`systemPrompt.ts:1-28`) previously called this an "Appendix A (25 rules verbatim)" prompt. It now honestly describes the current spine: **16** numbered CORE RULES, listed by name in the banner (13 through D7.1, +14 DPR-field authority / +15 claimed-current-term-change-is-an-assumption in the Phase-4 follow-ups, +16 what-if router in plan 35). The banner count is load-bearing — `systemPrompt.test.ts` derives the actual number of `N. <text>` lines in the CORE RULES block and asserts the banner agrees, so a future added/removed rule that doesn't update the banner FAILS that test. There is no 25-rule list in the emitted prompt; read the function body, not training-data memory of the old Appendix A.
...
1. **Core static rules** — sixteen numbered rules that are always present
2. **Preference extraction** — Phase-14 rules (reconciled in D6.1) about translating natural-language preferences into live `PlanMutation` entries for the `propose_plan_change` `mutations` array. This block also contains the **plan-37 confirm-narration rule** (I3: after `propose_plan_change`, the agent tells the student the proposal is visible on the canvas — they click Confirm or type a bare "confirm"/"yes" in chat; the agent does NOT call `confirm_plan_change` itself) and the **never-override rule** (M2: if `propose_plan_change` returns `feasible:false`, the agent explains WHY + what would make it valid + directs to an adviser for genuine exceptions — NEVER offers to "apply/override anyway"; the server refuses any commit of an infeasible plan and there is no override path).
...
## 1. The sixteen core static rules (always present)

These appear verbatim under the `CORE RULES (mandatory — non-negotiable):` header. Rules 9–11 are the **D4 honesty-rail** additions (2026-06-11); rule 12 is the **D6.5 honesty interlock** addition; rule 13 is the **D7.1 proactive-elicitation** addition; rules 14 (**DPR-field authority**) and 15 (**claimed-current-term-change-is-an-assumption**) are the **Phase-4 follow-up** additions (F2 / F3, 2026-06-16; rule 15 reworded for plan 35 §6, 2026-06-18); rule 16 (**what-if router**) is the **plan 35** addition (2026-06-18). The banner count is load-bearing — `systemPrompt.test.ts` derives the `N. <text>` line count + asserts the banner agrees (now **16**).
...
13. **Proactive elicitation (D7.1 + D7.2).** When a planning/decision question depends on decision-relevant context the student hasn't supplied — their intended **major/direction**, a **career interest/goal**, a **graduation timeline**, or (for a global-campus student — NYU Shanghai / NYU Abu Dhabi) **study-away intent** — the agent ANSWERS the question first with what it has, THEN appends **ONE** focused follow-up to gather the single missing fact, the way a professional adviser would. **Bounded:** at most one proactive question, sparingly; never substitute the question for the answer, never stack multiple asks, never interrogate/quiz; if the context is already known (program declared, interest stated, study-away chosen), do not ask. Framed as helping the student make a better decision, not a quiz. The directive is fed by a **pure deterministic detector** — `detectElicitationOpportunity` in [`proactiveElicitation.ts`](../../../packages/engine/src/agent/proactiveElicitation.ts) — the **proactive sibling** of the reactive clarifier's `detectAmbiguity` ([clarifier.md](clarifier.md)). The detector fires on three high-value/low-frequency signals (`planning_without_declared_program`, `major_exploration_without_interest`, `global_campus_planning_without_away_intent`), suppresses overlap with `detectAmbiguity` (the reactive clarifier owns ambiguous messages; this owns clear-but-context-thin ones), pure factual lookups, and continuations of a prior assistant question, and is **append-not-substitute**: it is consumed AFTER the agent loop completes, never streamed in place of the answer like the route clarifier. As of **D7.2** it is wired into `route.ts`: on the OK terminal path (`allOk`), the route calls `detectElicitationOpportunity` + the pure `decideElicitationAppend` and, when they fire, **appends** one bounded question (prefixed by the stable `ELICITATION_LEAD_IN` marker) to `finalTextOut` before the `done` event — so it rides into `done.finalText` (the client reconciles to it) and the durable transcript. The **bounded-frequency guard** reads that marker back out of the prior assistant turn in `history` (no two proactive asks in a row), and the same decision suppresses when the agent already asked (finalText ends with `?` or already contains the marker). The append is structurally substitution-proof (it only ever ADDS text) and try/catch-wrapped, so a detector failure can never break the live turn. See [`chat-route-sse.md`](../web/chat-route-sse.md) §11.

14. **DPR-field authority — read-only, change via re-upload (F2, 2026-06-16).** Fields the DPR shows DETERMINISTICALLY — home school, declared major/minor, catalog year, courses taken, grades — are authoritative from the student's DPR and CANNOT be changed by request. If a student asks to change one, the agent tells them to upload a corrected/new DPR rather than editing it; it never force-changes and never fabricates a value (e.g. a grade). Only NON-DPR fields (visa / F-1-vs-domestic status, preferences) can be confirmed/changed via the confirm flow. Enforced in depth: the v2 route ignores+doesn't-persist a `body.homeSchool` override when `deriveHomeSchool` confidently derives one (only the `"unknown"` fallback accepts a pick); the wizard shows a DPR-derived home school read-only; `updateProfile.ts` refuses catalogYear/declaredPrograms changes when a DPR is loaded + the value differs.

15. **Claimed current-term change is an UNVERIFIED assumption (F3, 2026-06-16; reworded plan 35 §6, 2026-06-18).** A CLAIMED drop / withdraw / pass-fail on a CURRENT-term in-progress course is an UNVERIFIED assumption — treated exactly as every in-progress course is already assumed to pass. The agent now COMPUTES its consequence deterministically (the engine's `applyWithdrawalToDpr` / `applyPassFailToDpr` transforms + `pfEligibility` — see [dpr.md](dpr.md)) and MAY let the student CONFIRM the resulting plan as a clearly-LABELED what-if — *confirming a PLAN is not recording a FACT*: the claim is NEVER folded into the authoritative DPR, the agent never fabricates the resulting grade/status, and a new real DPR re-plans + supersedes it. It surfaces the registration window (`classifyIpChangeability`) + the consequences: a "W" universally does NOT fulfill the requirement; **pass/fail is SCHOOL-SPECIFIC** (it satisfies a major at some schools e.g. Stern, electives-only at most, and the engine hedges where unknown); a P/F fail is not GPA-neutral. Closes with "verify with your adviser; nothing is official until it shows on your next DPR." A FUTURE / pre-registered IP course is freely changeable.

16. **What-if router (plan 35, 2026-06-18).** Classifies a what-if and routes the THREE branches: **(A)** a hypothetical PROGRAM change (declare/switch major, add minor, change school) → ask the student to upload their Albert What-If report (`/api/whatif-audit`), planned as a labeled NON-committed exploration (made real only by declaring it + uploading a new real DPR); **(B)** a current-term grade-outcome change (withdraw / pass-fail) → the rule-15 what-if-assumption flow (`propose_whatif_assumption` / `probe_counterfactual`); **(C)** anything else → the confidence-disclaimed read-only estimate tools (`what_if_audit` / `probe_counterfactual` / `simulate_alternatives`) + policy search.
...
> **Reconciled to the live schema (D6.1, 2026-06-11).** This table predated Phase 17 and had drifted. Until D6.1 it taught the kinds `load_style` (payload `{term, value}`), `include_summer`, `include_jterm`, `allow_below_floor`, and `set_scheduling_preference`. **Those kinds are NOT what `propose_plan_change` accepts** — the tool's `inputSchema` is `{ mutations: z.array(PlanMutationSchema).min(1) }`, and `PlanMutationSchema` ([`planChangeHelpers.ts`](../../../packages/engine/src/agent/forwardSchedule/planChangeHelpers.ts)) is a `z.discriminatedUnion("kind", …)` over a fixed kind set. There is **no alias/adapter layer** anywhere in `packages/engine/src` or `apps/web` that rewrites `load_style` → `loadStyleOverride` or `include_summer` → a preference field — grep confirms those names existed only in this prompt block and the matching eval fixtures. So before D6.1 the prompt was teaching the LLM to emit mutations the tool would reject at validation. (A separate `PlanChangeProposal` interface in `types.ts` used to carry the old `kind` strings, but `propose_plan_change` never consumed it; that dead interface was since removed — cleanup PR #47.) The table below is now the live union.
...
**The 5-bucket suite + how its bar is enforced (D6.6).** The suite is **5 buckets — A / B / C / Dpos / Dneg, ≥10 fixtures each** (currently 14 / 11 / 11 / 11 / 12 = 59 total), reconciled to the live `PlanMutation` union by D6.1–D6.4. Enforcement is split in two so the file behaves correctly with and without an API key:
- **Keyless CI (no `ANTHROPIC_API_KEY`)** — a `describe` of **unconditional invariant tests** pins *structure* (all buckets ≥10; every Tier-A fixture maps to a live `PlanMutation` kind incl. `move` + `unpin`; every Dneg fixture is `framing:"hard"` + `expectedTier:"C"`; ≥1 Dpos maps to `addSoftObjective` with `framing:"soft"`). These run on every `vitest run` and never call the LLM.
- **CI-with-key / local run** — an **operator-gated `it(...)` now asserts the ≥85%-per-bucket bar** (`expect(b.accuracy).toBeGreaterThanOrEqual(0.85)` per bucket), so a regression below 85% on any one bucket FAILS the test. The gate uses the repo's standard idiom (`describe` early-returns a single `it.skip` when no key is set — same as `explainWhy.eval.ts` / `honestyInterlock.test.ts`), with a generous per-test timeout (600 s) for the ~59 sequential real API calls. Before D6.6 the bar was only *logged* by an import-side-effect that could `process.exit(1)` during vitest collection; that side-effect is now guarded by `ANTHROPIC_API_KEY && !VITEST`, so it fires ONLY for the standalone `tsx` invocation, never under vitest.
...
- **Routing rules:**
  ...
  - **FOSE availability ≠ requirement satisfaction (K2 — critical guardrail, plan 37):** FOSE section availability governs only `materialize_sections` (meeting times / CRNs for the near term). It is **never** an input to whether a course satisfies a requirement or whether a plan is degree-feasible (the 7+1-axis validator's verdict). Requirement membership comes from the catalog (department + course-level range) and is FOSE-independent. Do NOT tell a student that missing or far-future FOSE sections mean a course "can't be confirmed" for a requirement. The validator and `run_full_audit` work purely from catalog data; `materialize_sections` is a separate, optional, downstream step that never feeds them.
  - **Optional terms (J-term / summer) are VALID even if they satisfy no requirement (L2, plan 37):** J-term and summer are never required for graduation; a student may open one for interest courses or a lighter schedule. When that term's courses satisfy no requirement, say so explicitly but do NOT flag the term or plan as infeasible. Never say a plan is infeasible because a J-term or summer term "can't be confirmed" for a specific requirement.
  - When `get_credit_caps` is called and its result is present, **quote its output verbatim** (e.g. "College of Arts and Science per-semester ceiling: 18 credits. F-1 full-time floor: 12 credits per semester.") rather than paraphrasing — verbatim quotes are enforced by the plan-claim validator (L2, plan 37).
  ...
  - After `propose_plan_change`, **narrate to the student that the proposal is now visible on the canvas — they can click Confirm there, or type a bare confirm phrase ("confirm", "yes", "apply it") in chat. Do NOT call `confirm_plan_change` yourself** — the canvas Confirm button is the single confirm chokepoint (I3, plan 37). Note: `confirm_plan_change` accepts only `{ mutations: [...] }` — there is no `pendingMutationId` parameter on that tool.
  - If `propose_plan_change` returns `feasible:false`, **explain WHY** (cite the specific failing constraint by name) + **state what would make it valid** + for a genuine policy exception direct to an adviser. **NEVER offer to "apply anyway" or "override"** — the server refuses any commit of an infeasible plan; there is no override path (M2, plan 37).
...
flowchart TD
    ...
    ROLE --> CORE[16 core static rules]
