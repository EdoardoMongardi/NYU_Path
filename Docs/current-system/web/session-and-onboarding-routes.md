...
## 6. Onboarding / preference wizard — LIVE (the `awaiting_dpr` onboarding surface)

**Status (binding honesty — F1, `7835788`):** the `OnboardingWizard` is **MOUNTED + LIVE**. It is rendered for the `awaiting_dpr` onboarding state in `apps/web/app/chat/page.tsx:1863-1867` (`{onboardingStep === "awaiting_dpr" && <OnboardingWizard onReachPlan={handleWizardReachPlan} onPreviewIntendedMajor={handleIntendedMajorPreview} />}`), **replacing the legacy welcome/upload ask-and-reply UI for that state** (the old "welcome" bubble + free-text upload prompt is suppressed once `onboardingStep === "awaiting_dpr"` — `page.tsx:1609`). Other onboarding states (`confirming_data` / `asking_visa` / `asking_graduation` / `complete`) and the session-restore path are unchanged. The handoff to the chat (the cutover seam) is now the wizard's button-triggered `onReachPlan` (`handleWizardReachPlan`) — see [§6.7](#67-the-wizard--chat-handoff-handlewizardreachplan-f1). Earlier this section read "built + tested, mount deferred"; that is superseded — the wizard is the live onboarding surface.

**Render harness (F1):** mounting added a DOM render-test rig — dev deps `@testing-library/react` + `jsdom` (owner-approved), with jsdom scoped **per-file** to `*.render.test.tsx` via a `// @vitest-environment jsdom` pragma (the vitest `include` glob now matches `.test.tsx`). First render tests: `apps/web/tests/mountedWizard.render.test.tsx` (the wizard mounts at `awaiting_dpr`) + `apps/web/tests/wizardHandoff.render.test.tsx` (the step flow → "Build my plan" → handoff). The rest of the suite stays in node.
...
| Home-school propose+confirm (+ F2 `derivedFromDpr`) | `apps/web/lib/wizard/homeSchool.ts` | pure helpers |
...
| Mount + chat-page consumers | `apps/web/app/chat/page.tsx:1863-1867` (mount) · `:885-929` (`handleWizardReachPlan`) · `:832` (`handleApplyWizardPreferences`) · `:704` (`handleIntendedMajorPreview`) | LIVE wiring |
...
- `computeHomeSchoolProposal(dpr)` (`homeSchool.ts:99-112`) — PROPOSES the DPR-derived home school by REUSING the real derivation (`buildStudentProfileFromDpr(dpr).homeSchool` → `deriveHomeSchool`). When the derivation degrades to the school-agnostic `"unknown"` (or empty), it returns `{ proposed: null, needsPrompt: true, derivedFromDpr: false }` — the wizard renders an explicit prompt with **NO default pre-selected** (the ONLY editable home-school case). **`"unknown"` NEVER becomes `"cas"` here.** When the DPR confidently derives a school it returns `{ proposed: <code>, needsPrompt: false, derivedFromDpr: true }` — **F2: the Confirm-profile step then renders that school READ-ONLY** ("from your DPR — upload a corrected DPR to change"), with NO editable picker. `derivedFromDpr` is the SAME authority signal the v2 route gate uses ([§6.6](#66-dpr-derived-field-enforcement-read-only--re-upload-redirect-f2)), so the UI and the server agree on what's overridable.
- `isValidSchoolCode(code)` (`homeSchool.ts:115-117`) — membership in `SCHOOL_OPTIONS`.
...
### 6.6 DPR-derived field enforcement — read-only + re-upload redirect (F2)

**F2 (`c0fbb7d`, owner decision #1 of [plan 34](../../plans/34-2026-06-16-phase4-pre-merge-followups.md)) — the fields the DPR shows deterministically are READ-ONLY.** Home school, declared major/minor, catalog year, courses taken, and grades come FROM the DPR and CANNOT be changed via the wizard or by asking the agent. To change one, the student uploads a **corrected / new DPR**. The ONLY editable identity/profile cases are: **home school when the DPR can't determine it** (the `deriveHomeSchool → "unknown"` fallback), **visa status** (never on a DPR), and **preferences**. The enforcement lives at four seams:

1. **The v2 route home-school gate** (`apps/web/app/api/chat/v2/route.ts:247-274`). `deriveHomeSchool` is now EXPORTED from `lib/buildSession.ts` and is the AUTHORITY signal. The route computes `dprDerivedHomeSchool = deriveHomeSchool(parsedDpr)` and `dprConfidentlyDerivedSchool = dprDerivedHomeSchool !== "unknown"`. When the DPR confidently derives a school, the route sets `validatedHomeSchool = undefined` — it **ignores and does not persist** a `body.homeSchool` override (logging a warning that points the student at a re-upload), and the DPR-derived value is used downstream. The override is honored ONLY when the DPR derived `"unknown"`. (This single narrowing cascades: the constructor `homeSchoolOverride` won't apply, and the E5.2 change-persist won't fire.) See [build-session.md §`deriveHomeSchool`](build-session.md#deriveh­omeschool).
2. **The wizard Confirm-profile step** renders a confidently-derived home school READ-ONLY via `computeHomeSchoolProposal(dpr).derivedFromDpr === true` ("from your DPR — upload a corrected DPR to change", no picker). It prompts for a pick ONLY on the `"unknown"` fallback ([§6.3](#63-confirm-profile--homeschool-propose--confirm-e52--visastatus-persist-e53)).
3. **The agent — CORE RULE 14** (`packages/engine/src/agent/systemPrompt.ts`): the agent treats DPR-derived fields as authoritative + read-only, **redirects a change request to "upload a corrected / new DPR"** (never force-changing, never calling `update_profile` for them), and NEVER fabricates a DPR value (e.g. a grade the DPR doesn't show). Only visa/F-1 status and scheduling preferences are editable by request. See [engine/system-prompt.md](../engine/system-prompt.md).
4. **The `update_profile` tool hard guard** (`packages/engine/src/agent/tools/updateProfile.ts:132-172`, defense-in-depth): when a DPR is loaded (`session.degreeProgressReport`), the tool REFUSES a `catalogYear` or `declaredPrograms` change whose proposed value DIFFERS from the DPR-derived value on `session.student`, returning the "upload a corrected DPR" message. An identical (no-change) value passes. `homeSchool` is deliberately NOT guarded here (it has the cleaner "unknown → student may pick" exception owned by the route gate + CORE RULE 14); `visaStatus` stays editable (never on a DPR).

### 6.7 The wizard → chat handoff (`handleWizardReachPlan`, F1)

`handleWizardReachPlan(values, dpr)` (`page.tsx:885-929`) is the wizard's `onReachPlan` consumer — the cutover seam, now LIVE. It fires from the wizard's **"Build my plan" button** at the terminal `plan` step (the student sees the plan step and confirms — not an automatic jump). On handoff:

- **With a DPR present** → `setParsedData({ kind: "dpr", report: dpr })` + `setOnboardingStep("complete")` (mirroring the session-restore path), so EVERY future chat turn routes through the v2 agent loop (the `useV2` gate is `onboardingStep === "complete" && parsedData`) and the sidebar hydrates. It seeds the chat-page profile state (`setHomeSchool` only when chosen — never silent CAS; `setVisaStatus`; `setGraduationTarget`) and applies Goals + Preferences onto the Phase-3 ladder via `handleApplyWizardPreferences`. To defeat the stale-closure race (the setters are async), it threads the just-confirmed DPR + values EXPLICITLY through `SendSeed.parsedData` so the FIRST injected preference turn already carries the DPR instead of a pre-seed closure value.
- **With NO DPR** → it does NOT complete (core philosophy: no DPR → no personalized plan). It surfaces an "I need your Degree Progress Report to build your plan — please upload it" message and stays `awaiting_dpr`, **fabricating no plan**.
