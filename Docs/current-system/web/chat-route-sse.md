...
> Last verified against code: 2026-06-19 (Plan 37: `plan_proposal` added to SSE event union table §10; `whatif_audit_request` verified present). Prior 2026-06-15: Phase 4 E1: profile read-back — new §5.5 documents the confirmed-profile read-back into `session.student` (E1.2: 4 editable fields, precedence body>persisted>DPR, ordered before schoolConfig load); the top honest-caveat + §5.1 revised since profile read-hydration is no longer deferred. Same-day prior pass: cohort gate subsystem removed — the per-turn cohort lookup, the `cohortLookup` store field, and the recovery/limited-availability path are all gone; the agent loop now runs unconditionally every turn. Earlier 2026-06-13 pass: refreshed `route.ts` line citations and added the post-loop proactive-elicitation append on the OK terminal path; see §11.
...
> **Profile read-hydration — the confirmed fields are now read back (Phase 4 E1.2).** As of **P3.1**, this route hydrates the persisted forward schedule, draft plan, and scheduling preferences from Postgres each turn (`scheduleStore.loadLatestSchedule` + `loadPreferences`), so the agent sees the plan the student built. **P3.2** then closed the profile *write*-clobber: the bootstrap upsert is gated to initial onboarding, so a confirmed profile mutation is no longer overwritten on every message. **Phase 4 E1.2** closes the deferred READ side: after rebuilding `StudentProfile` from the re-sent DPR, the route reads `profileStore.get(userId)` and merges the **four `confirm_profile_update`-editable fields** (`homeSchool` / `catalogYear` / `declaredPrograms` / `visaStatus`) back into `session.student` — so a confirmed profile edit (e.g. a corrected home school) survives into the agent's view each turn instead of being clobbered by the fresh body-DPR derivation. The body DPR stays authoritative for course history / standing / credits (the whole persisted profile is NOT blindly assigned). See [§5.1 Per-turn plan + prefs hydration](#51-per-turn-plan--prefs-hydration) and [§5.5 Confirmed-profile read-back into `session.student`](#55-confirmed-profile-read-back-into-sessionstudent-e12). The `/api/plan/*` orchestrator and [`/api/session/restore`](session-and-onboarding-routes.md#2-get-apisessionrestore) hydrate fully from Postgres (profile included).
...
**Profile write-clobber — fixed in P3.2; confirmed-field read-back added in E1.2.** §5.1 is READ-hydration of the plan + prefs. The route still rebuilds the *base* `StudentProfile` from the **client-resent** DPR every turn, but **Phase 4 E1.2** now reads the persisted profile via `profileStore.get` and merges the four `confirm_profile_update`-editable fields back onto `session.student` (see [§5.5](#55-confirmed-profile-read-back-into-sessionstudent-e12)). The *write* side was already fixed in P3.2: the bootstrap upsert below (§5.3) is **gated to initial onboarding** (`profileStore.get(userId) === null`), so it no longer rewrites `students.profile` on every message and no longer **clobbers confirmed profile mutations** a prior `confirm_profile_update` wrote, nor appends a synthetic audit row per message. The related preferences-from-`{}` wipe is also resolved: a chat-driven `confirm_plan_change` now reads the P3.1-hydrated `session.schedulePreferences` (not `{}`), so existing pins survive.
...
### 5.5 Confirmed-profile read-back into `session.student` (E1.2)

**Phase 4 E1.2** closes the profile READ-back that P3.2 deliberately deferred. The route rebuilds `student` fresh from the re-sent body DPR every turn (`buildStudentProfileFromDpr`, `route.ts:214-229`), so a confirmed `confirm_profile_update` edit was persisted to `profileStore` but invisible to the agent. Immediately **after** `student` is built and **before** `loadSchoolConfig(student.homeSchool)` runs (the `schoolConfig` IIFE at `route.ts:308-314`), the route reads `stores.profileStore.get(userId)` and merges back **only the four fields `confirm_profile_update` can mutate** (the `PendingProfileMutation["field"]` union) — `homeSchool`, `catalogYear`, `declaredPrograms`, `visaStatus` (`route.ts:232-301`). The body DPR stays authoritative for course history / standing / credits, so the whole persisted profile is **not** blindly assigned (it may carry a stale DPR snapshot).

- **Precedence (freshest explicit signal wins):** an explicit value the **client** sent THIS turn beats the persisted value, which beats the DPR derivation. For `homeSchool` / `visaStatus` the constructor overrides above already reflect a body-sent value, so persisted applies only when the body did **not** send a usable value (`bodySentHomeSchool` / `bodySentVisa` guards). The body never sends `catalogYear` / `declaredPrograms`, so persisted (when present / non-empty) always overrides the DPR-derived value.
- **Critical ordering:** this runs **before** `schoolConfig` is loaded — `schoolConfig` keys on `student.homeSchool`, so a confirmed home school MUST land here or every school-scoped tool would silently use the wrong school (a binding "never silently CAS" concern).
- **Guarded + best-effort:** skipped for `userId === "anonymous"` (no durable row), wrapped in try/catch — a load failure is `console.warn`ed and the turn continues with the DPR-derived values. This is a **separate single-PK read** from the P3.2 write-gate's existence check ([§5.3](#53-bootstrap-profile-persistence)), which is left untouched.

The matching parity exit-gate (`apps/web/tests/chatSidebarParity.test.ts`) proves the sidebar HTTP edit path and the chat/agent (`planActionOrchestrator`) edit path converge on ONE persisted `forwardSchedule` row, plus an in-session `createPlanStore` render-parity check (see [chat-ui-client.md](chat-ui-client.md)).
...
| `whatif_audit_request` | `hypotheticalProgram` (string) | Plan 36 H4.2b — Branch-A audit-upload offer. Emitted once per turn when the agent calls `what_if_audit` and its summary carries an `AUDIT_UPLOAD_OFFER: <label>` marker. The client renders a `whatif_upload_card` prompting the student to attach the Albert What-If audit for the hypothetical program. |
| `plan_proposal` | `pendingMutationId`, `feasible` (boolean), `consequences[]` (string[]), `proposedSchedule?` (ForwardSchedule), `planDiff?` (PlanDiff) | Plan 37 I1 — chat-confirm bridge. Emitted at most ONCE per turn when the agent called `propose_plan_change`. Carries the staged `pendingMutationId` + the resulting schedule + feasibility signal. **Feasible** → client creates a proposed scenario tab + Confirm rail (no extra "Confirm?" agent message needed). **Infeasible** → client shows the red explanation card only (no Confirm, no scenario tab). Staging ≠ committing — nothing persists until the student clicks Confirm or types a confirm affirmation. Handled by `applyPlanProposalEvent` (`apps/web/app/chat/planProposalEvent.ts`). |
...
sequenceDiagram
    ...
    Route->>Stores: profileStore.get → merge 4 confirmed fields back into session.student (E1.2)
    Note over Route,Session: P3.1 reads persisted schedule + prefs; P3.2 gates the profile upsert to initial onboarding; E1.2 reads the 4 confirmed profile fields back into session.student (before schoolConfig load)
