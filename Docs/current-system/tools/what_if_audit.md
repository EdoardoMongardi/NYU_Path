...
> Last verified against code: 2026-06-19 (plan 37 — I4: when `what_if_audit` returns and the Branch-A upload produces an **invalid** what-if scenario (the synthetic DPR yields an infeasible plan), that result is **chat-only** — no workspace scenario tab is created for it; the student sees the red explanation card in chat but cannot confirm it. A **valid** Branch-A audit result does produce a labeled read-only `kind:"whatif"` workspace scenario (no `pendingMutationId`, never confirmable — view-only). This distinction between invalid=chat-only and valid=read-only-scenario is enforced at the web route level).
>
> **Place in the plan-35/37 what-if taxonomy:** `what_if_audit` is **Branch C** — the confidence-disclaimed read-only ESTIMATE for an open hypothetical (it never ingests a file). Three branches exist: **Branch A** = a hypothetical PROGRAM change resolved by UPLOADING the student's Albert What-If report (`/api/whatif-audit` → parses as a synthetic DPR → labeled NON-committed read-only exploration; never overwrites `students.parsed_dpr`; invalid result = chat-only per I4); **Branch B** = a current-term withdraw/pass-fail modeled deterministically via the `propose_whatif_assumption` confirmable flow (or `probe_counterfactual` for read-only introspection; see [propose_whatif_assumption](./propose_whatif_assumption.md) and [../engine/dpr.md](../engine/dpr.md)); **Branch C** (this tool) = anything else → confidence-disclaimed estimate. CORE RULE 16 routes between them.
