...
- **Chat now hydrates the persisted plan + preferences each turn, but not the profile.** As of **P3.1**, each chat turn loads the previously-saved `forward_schedules` / `schedule_preferences` rows back into `session.forwardSchedule` / `session.studentDraftPlan` / `session.schedulePreferences` at the start of the turn (`scheduleStore.loadLatestSchedule` + `loadPreferences` in `apps/web/app/api/chat/v2/route.ts`), so a plan built in an earlier turn — or via the sidebar — is in scope for later turns and the agent no longer runs blind to it. The **profile** is still rebuilt from the student's re-sent DPR every turn rather than read from `profileStore.get`, and the bootstrap upsert can still clobber confirmed profile mutations — fixing that profile-clobber (plus the related preferences-from-`{}` wipe on chat-driven confirms) is the separate **P3.2** task.
