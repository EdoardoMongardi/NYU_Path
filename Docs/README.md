...
| [`reference/`](reference/) | Policy & provenance reference (privacy, model selection, data-source logs) | n/a |
...
- `tools/` — one doc per live agent tool (20 registered in `packages/engine/src/agent/registry.ts`).
...
These were first written 2026-06-03 (pre-rebuild) and are being revised against the post-rebuild code (planning-engine rebuild Phases 0–2, merged via PRs #35–#41, June 5–7 2026). A doc's header notes its last code-verification date.
...
2. **20–31 — rebuild era (June 2–7 2026):** `20-improvement-plan.md` (the post-audit roadmap, Phases A–F incl. the pure-RAG decommission) → SPS division router → **planning-engine rebuild**: foundation (Phase 0/1, PR #35) → phase-2 solver (PR #36) → solver tractability (PR #38) → diverse-plans perf (PR #40) → double-count advisory (PR #41). Cut-maps and handoffs are filed next to their plan.
...
- `2026-06-05-planning-engine-rebuild-design.md` — **the canonical rebuild design** (6 layers, Phases 0–5). Phases 0–2 are implemented; §7.2's branch-and-bound search was superseded in implementation by feasibility-first search + local improvement (PR #38).
