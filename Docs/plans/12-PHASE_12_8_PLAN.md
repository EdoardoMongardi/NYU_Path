...
5. "Minimum Grade of C" annotations are IGNORED in the LLM parser
   pass — they're extracted separately by
   `tools/bulletin-parser/extractGradeThresholds.ts` (regex-only) and
   stored at the entry level as `minGrades: Record<courseId, grade>`,
   not inside any PrereqGroup. Phase 13's solver consumes
   `Prerequisite.minGrades` via `packages/engine/src/dpr/
   prereqSatisfaction.ts` per the now-current Decision #4
   (optimistic-forward-projection with explicit grade thresholds).
   The original instruction here said "DPR handles grade thresholds;
   we don't model them" — that decision was reversed 2026-05-03 after
   the silent-bug risk on B/B+/A- thresholds surfaced.
