...
> **Status:** in progress. **Phases A ✅, B ✅ (core), C ✅, D ✅, E ✅ (E1+E2) are merged. Phase F (decommission) is well underway** under the user's **"remove all — nothing hardcoded"** decision (which supersedes the older "keep `cas_to_stern`" note in the Phase F table below — the live RAG eval confirmed `search_policy` answers transfer well, so the authored route was removed). Each removal ships as its own small PR.
>
> **Pure-RAG decommission progress (current):**
> - ✅ Reliability + model: RAG-path rate-limit backoff + output-truncation stitch fix; **primary switched Haiku → `claude-sonnet-4-6`** (Haiku was variable on hard multi-part policy; same retrieval, model-capability gap) — PR #17. Plus a `stores` session-summary bug fix — PR #20.
> - ✅ **Removed `check_transfer_eligibility`** + `data/transfers/*.json` + `transferPrepPlanner` → internal transfer is pure `search_policy` RAG — PR #18.
> - ✅ **Removed `check_overlap`** → double-counting POLICY via `search_policy`, the student's per-program satisfaction via `run_full_audit` (DPR) — PR #21.
> - ✅ **`what_if_audit` → pure RAG estimate** (dropped the authored rule-engine path); course-level "what if I take A instead of B" routes to the solver via `propose_plan_change`/`simulate_alternatives` — PR #22.
> - ⏳ **8b (next, the big one):** delete the rule engine (`degreeAudit`/`ruleEvaluator`/`crossProgramAudit`/`whatIfAudit` engine) + the dead-planner library (`semesterPlanner` et al.; **keep `graduationRisk`**) + `programs.json`/`loadPrograms` + the **vestigial** `session.programs` wiring (verified: `buildProgramRules` derives from `schoolConfig`+`dpr.requirementGroups`, not `session.programs`). ~19 test files; no planner surgery. Then re-test live on Sonnet.
> - ⏳ **8c:** trim `schoolConfig`'s DPR-duplicated fields (keep the registration constants — ceiling + F-1 floor — in `data/schoolDefaults.ts`).
> - ⏳ **8d:** `loadCourses` → the **full 17,122-course catalog that already exists** at `data/course-catalog/course_descriptions.json` (used by `search_courses`) + a shape adapter. Wiring, not data-sourcing; decoupled from the rule-engine removal.
>
> **Known planner bugs (separate, tracked in GitHub #19):** the forward solver enforces no double-count limit and its greedy skip (`solver.ts:889`) can silently hide an unmet requirement. To fix together later — independent of the decommission.
>
> This plan turns the audit findings into a phased, buildable roadmap. Effort sizes are rough (S = hours, M = a focused day or two, L = multi-day + ongoing data work).
...
|---|---|---|
| Deprecated single-term planner: `semesterPlanner`, `balancedSelector`, `priorityScorer`, `multiSemesterProjector`, `crossProgramPlanner`, the `plan_semester` tool, the `planFeasibility` verifier | Fully superseded by the forward-schedule solver; `plan_semester` is already unregistered | ✅ `plan_semester`+`planFeasibility` removed (Phase F start). The **library** (`semesterPlanner`, `crossProgramPlanner`, `balancedSelector`, `priorityScorer`, `multiSemesterProjector`, `enrollmentValidator`, `explorePlanner` — **KEEP `graduationRisk`**) is removed in **8b**, entangled with the rule engine. |
| Orphaned engine `transcript/` module | Dead, test-only; production uses the DPR parser | None — but needs surgery across ~6 mixed eval-test files (still pending; out of the current decommission scope) |
| Authored rule engine: `degreeAudit`, `ruleEvaluator`, `crossProgramAudit` + authored program JSON (`data/programs/`) + `programLoader` / `catalogYearLoader` | Only live consumers were `check_overlap` + the `what_if` authored path + the deprecated planner | **✅ gate cleared** — `check_overlap` (PR #21) + `what_if` authored path (PR #22) migrated. **→ 8b (next).** `session.programs` verified vestigial (`buildProgramRules` uses `schoolConfig`+DPR). |
| `schoolConfig` DPR-duplicated fields + their branches in `get_credit_caps` / `creditCapValidator` | The DPR carries these per-student | **✅ Phase E done → 8c** (keep the registration constants — ceiling + F-1 floor). |
...
- **`schoolConfig` registration constants** (per-semester ceiling + F-1 floor, now in `data/schoolDefaults.ts`) — the live planner needs them; the DPR doesn't carry them.
- **`graduationRisk`** — independent + barrel-exported; keep when deleting the rest of the planner library in 8b.
- ~~The deterministic `cas_to_stern` transfer checklist~~ — **REMOVED (PR #18)** per the user's "remove all — nothing hardcoded" decision. The live RAG eval confirmed `search_policy` over the internal-transfer bulletin pages answers transfer for *every* school, so the single hardcoded route was redundant (this supersedes the original "keep it as Tier-1" note).
