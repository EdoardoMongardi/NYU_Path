...
>
> **DEFERRED (own follow-on tasks, NOT before this merge):** (1) the precise W / pass-fail → requirement-satisfaction engine modeling (F3 hedges it in prose; solver/validator untouched); (2) owner-fill of the absent academic-calendar dates — NY Summer-2026 + Spring-2027 add/drop+withdraw deadlines and the NYU Shanghai / Abu Dhabi maps (each unfilled term/campus correctly falls back to the generic hedge). **OPTIONAL (owner-confirm, per philosophy #5):** `core_philosophy.md` could be extended to encode the DPR-derived-field-authority + IP-window rules — left UNCHANGED pending owner sign-off.
