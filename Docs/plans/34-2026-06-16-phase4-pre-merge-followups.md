...
> Drafted 2026-06-16. **Status: COMPLETE** (F2 `c0fbb7d` · F3 `d592219` · F1 `7835788`, all subagent-driven TDD + spec + code-quality reviewed; suite 2102 passed / 15 skipped; `apps/web` + `packages/engine` tsc clean). Owner-directed follow-ups to `33-2026-06-15-planning-engine-phase4-experience.md`, landed on `feat/phase4-experience` BEFORE merge.
>
> **As-built:** F2 — DPR-derived fields (home school/major/minor/catalog year) are read-only; the agent redirects change requests to a corrected-DPR upload (CORE RULE 14 + route gate + `updateProfile` guard); the wizard shows a DPR-derived home school read-only; only the unknown-school fallback + visa + preferences are editable; nothing fabricated. F3 — IP-course changeability is window-aware + verification-grounded: future-term IP freely changeable, current-term IP classified against NYU's cite-or-hedge academic-calendar windows, any claimed change is an unverified hedged assumption never recorded as fact (CORE RULE 15). F1 — the structured `OnboardingWizard` is MOUNTED as the live `awaiting_dpr` onboarding (button-triggered handoff sets `parsedData`+`complete`, threads the DPR through `SendSeed`; no DPR → upload-needed message, no fabricated plan); first DOM render tests via `@testing-library/react`+jsdom (per-file env).
>
> **F3-campus (`975e42c`, owner-directed, landed before merge):** NYU Shanghai + Abu Dhabi now have their OWN sourced per-season patterns in `academicCalendar.ts` (`SHANGHAI_SEASON_WINDOWS` / `ABU_DHABI_SEASON_WINDOWS`) instead of sharing the NY defaults — fall/spring sourced from each campus's registrar (cited), genuinely-unsourced windows (Shanghai's TBD spring withdrawal, both campuses' summer/J-term deadlines) left ABSENT → honest hedge (never invented). `core_philosophy.md` line 18+ WAS updated (owner-approved) with the DPR-field-authority + IP-window rules.
>
> **DEFERRED (own follow-on tasks, NOT before this merge):** (1) the precise W / pass-fail → requirement-satisfaction engine modeling (F3 hedges it in prose; solver/validator untouched); (2) owner-fill of the remaining absent calendar windows — NY Summer + J-term are reasonable estimates; Shanghai's spring full-term withdrawal (published TBD) and both non-NY campuses' summer/J-term add/drop+withdraw deadlines are still ABSENT and fall back to the generic hedge until sourced.
