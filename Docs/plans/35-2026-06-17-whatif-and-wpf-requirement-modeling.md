...
> Drafted 2026-06-17. **Status: PLAN / DESIGN — awaiting owner sign-off on the CORE RULE 15 refinement (§6) before implementation.** Resolves the long-deferred follow-on from Phase 4 (`Docs/plans/34-*`): "W / pass-fail → requirement-satisfaction engine modeling (does a W satisfy requirement R; does P/F satisfy a letter-grade major rule)." Every claim below was verified against the real `NYU_Path` (underscore) code at HEAD `d5e757c` by a 15-agent investigation + adversarial verification + a 2-agent bulletin gap-fill; load-bearing facts carry `file:line` cites.
>
...
- **The What-If audit is FIELD-COMPLETE vs a normal DPR (verified 2026-06-17 by parsing both fixtures side-by-side).** Every field the engine consumes is present and populated in the what-if parse: `header.studentName`; `programs` (incl. the home-school label `UA-Coll of Arts & Sci` — so `deriveHomeSchool` still works); the FULL `cumulative` block (creditsRequired/Used 128/142, cumulativeGpa 3.481/2, residency 64/80, passFailUsed/Cap 8/32, outsideHomeUsed/Cap 18/16, timeLimitYears 8, `residencyAll`); `requirementGroups` (12, with **more** leaves — 38 vs 33 — reflecting the hypothetical major, `notSatisfied` 10 vs 2); full `courseHistory` (38 rows, 34 with grades). **There is NO missing-crucial-data risk** — the what-if is structurally a complete DPR for the hypothetical scenario. ⚠️ It does throw a few EXTRA non-fatal parser warnings (a mis-detected course-table header row; two `Course Topic:` / `Repeat Code:` continuation lines; a `Page N of M` prefix leaking into a section title → that parent group defaults to "satisfied" while its leaves keep their real statuses). None drop data, but Branch A should harden these (G4.3).
...
## 6. The one philosophy refinement to confirm (philosophy #5 — owner sign-off required)
...
**This is the only item gating implementation.** Everything else is verified-safe.
...
- **G4.1** Upload route accepts a What-If PDF (reuse the `/api/onboard` extract+parse), detects `reportKind:"what_if"` (G0.1), plans against it as a **what-if plan** (label "hypothetical <program>"), and **never** calls `persistStudent`/writes `parsed_dpr` (G0.2 guard). *Test:* uploading the what-if fixture yields a plan for the hypothetical program; `parsed_dpr` unchanged.
