...
### Task I4: an INFEASIBLE chat what-if still renders as a comparable (invalid) scenario

**Finding (verified):** `planActionSurfaces` (`apps/web/lib/planActionSurfaces.ts:69-75`) maps `feasible:false` → `{ preview:null, invalidCard: computeInvalidCard(...), showBubble:true }` — a RED card + a chat bubble, NOT a scenario. So an INVALID what-if never enters `scenarios[]` and cannot be opened or compared in the workspace. In the J-term test the student asked to "create a what-if to compare," the solver flagged it infeasible, and it surfaced only as prose + a red card — nothing to compare. We want an invalid what-if to STILL be a comparable, red-badged scenario (scenarioModel already allows `verdict:"invalid"`).

**Files:** Modify the I2 `plan_proposal` handler in `page.tsx` + `apps/web/app/chat/workspace/CompareView.tsx` (allow invalid scenarios in the pickers). Test: extend the I2 test + a CompareView render test.

- [ ] **Step 1 — failing test:** an infeasible `plan_proposal` event yields a scenario in `planStore.getScenarios()` with `verdict:"invalid"` (a red-badged tab), AND that scenario is selectable in `CompareView` (the picker does not exclude `verdict:"invalid"`).
- [ ] **Step 2/3 — implement:** in the I2 handler, on the `feasible:false` path, `planStore.addScenario({ kind:"proposed", verdict:"invalid", schedule: proposedSchedule, hedges:[…consequences], … })` (carrying the infeasible proposed schedule) IN ADDITION TO setting the red invalid card as the action-body verdict — so it appears as a red tab the student can open + select in compare. Confirm `CompareView`'s pickers + `scheduleDiff` handle an invalid scenario (they diff schedules regardless of verdict). The proposed scenario keeps NO confirmable affordance beyond the existing invalid-path "Override anyway" (it's invalid) — comparing it is read-only.
- [ ] **Step 4 — run PASS + full suite. Step 5 — commit.**
