...
> **COMPLETE — 2026-06-19.** Branch `feat/plan37-slot-editor`, 42 commits, suite **2642 passed**, engine + web `tsc --noEmit` clean. Frozen contract intact (solver/`finalizeForwardSchedule`/search untouched; only the validator gained an 8th axis — owner-approved). Key owner decisions made during execution: (1) Branch-B (withdraw / pass-fail) what-ifs are **confirmable** (per-owner reversal of I4's initial read-only treatment; audit/Branch-A what-ifs remain read-only); (2) the P/F-eligibility gate (`canElect:false`) is enforced **server-side** covering BOTH the agent path and the route/editor path — the client matrix may leave the P/F button un-grayed at a `canElect:false` school, but the server rejects it (not a correctness hole); (3) `force`/Override-anyway is **retired** — an infeasible re-solve returns HTTP 422 and the prior valid row survives unchanged; `student-preferred-invalid-draft` is no longer minted (retained only for display back-compat); (4) per-term P/F limit enforcement is NOT wired in the shipped axis (career cap only — see open follow-ups in D-4).
...
- **per-term limit** (`perTermLimit` over `perTermUnit` semester/academic_year) → HARD on the elections WE can see (our P/F elections in a term/year); HEDGED for any existing per-term P/F the DPR doesn't expose (the DPR carries only the career total). ⚠ **NOT YET WIRED in the shipped C1/C2 axis (verified 2026-06-19): `perTermLimit`/`perTermUnit` have zero readers — the committed axis enforces only the CAREER cap. See "Open follow-ups" below.**
...
- **`canElect: false`** (Tandon) → the pass/fail **election** action is disabled (the matrix forbids *electing* P/F before the axis is reached). **Precise rule** (bulletin `engineering/academic-policies/_index.md:158-161,220`): a Tandon student **cannot elect** to convert a letter-graded course to P/F — but a course that is **inherently P/F-only for everyone** is still allowed and counts **only as a free elective** (P = D-or-above, earns credit, GPA-neutral; F earns no credit but counts in GPA). So it is NOT "Tandon has no P/F" — it is "no *student-elected* P/F." (The `tandon.json` `passFail.note` + the matrix's forbidden-reason copy at `slotActionMatrix.ts:144` already state this correctly; this bullet's wording is corrected to match.)
...
**Cross-school governance — whose P/F rule applies to an outside-school course (verified 2026-06-19 against the raw bulletin).** P/F eligibility keys on the **student's HOME school** config (`pfEligibility(schoolId, category)` — `pfEligibility.ts:117`; the offering school of an individual course is never consulted). That is **correct for the dominant bulletin pattern**: CAS, Stern, Tisch, Steinhardt, LS, SPS, Tandon, and NYUAD govern P/F by the student's own school regardless of where the course is offered — stated explicitly in the bulletin (Stern: *"Stern students petition to elect classes (**including non-Stern**) pass/fail"*; Tandon: *"…pass/fail courses **in other divisions of NYU** only if…"*). A course counting toward a **major or minor in ANY school** is not P/F-eligible — the engine already enforces this conservatively via `countsForMajor`/`countsForMinor` + `classifyLeafCategory` (CAS: *"No course to be counted toward the major or minor may be taken on a Pass/Fail basis"*). **Known limitation (cite-or-hedge):** two schools flip to the **offering** school — **Nursing** (*"based on the policies of the school or college in which the course is being offered"* — already noted in `nursing.json` `passFail.exceptions[]`) and **Gallatin** (files with Gallatin, but the offering school's eligibility can differ); **GPH** defers to the home school. For a *cross-school* election by a Nursing/Gallatin student the home-school-only model may be imprecise, so the agent must **hedge** ("your home school's limit applies to all your P/F elections, but Nursing/Gallatin base eligibility on the offering school — verify with your adviser") rather than assert. No frozen-contract change and no code restructure for the dominant case; the home-school model stays.
...
### Task K2: system-prompt guardrail — FOSE availability is never a requirement-satisfaction input  ✅ DONE (refined 2026-06-18)

**Files:** Modified `packages/engine/src/agent/systemPrompt.ts` (the FOSE-availability bullet — landed near `:557`, not the originally-scoped `:538`). Test: the existing prompt/eval guards + `tests/agent/systemPrompt.test.ts` (28/28 green); no new runtime test (prompt text only). Engine `tsc --noEmit` clean.

- [x] **Step 1 — implement:** added the explicit rule under the FOSE note. **Refinement during execution (owner-raised, 2026-06-18):** the originally-planned wording said FOSE "is NEVER an input to whether a course satisfies a requirement **or whether a plan is feasible**." The bare word *feasible* over-reached — it conflated two distinct feasibility layers and literally contradicted the project's own binding docs (the canonical spec's `(FOSE-light) no time-conflict / not closed` HARD validity axis; `core_philosophy.md`'s "avoid time conflicts when FOSE is available"; the named Phase 15 "Live Section Materialization (FOSE) + Time-Conflict Detection" plan). It would also mislead the future FOSE phase (the agent could wave off a *real* time conflict as "not a feasibility concern"). So the **shipped** wording **scopes "feasible" → DEGREE-feasible** (requirement coverage / credits / prereqs / graduation target — the 7+1-axis validator's verdict) and **adds a paragraph naming SECTION-SCHEDULING feasibility as a SEPARATE downstream layer that FOSE legitimately governs** (meeting-time conflicts / closed sections, near registration term only; operates ON an already requirement-valid plan; may prompt a re-plan; NEVER changes whether a course counts toward a requirement). This keeps the guardrail's intent intact (FOSE ≠ requirement satisfaction — no "no Spring 2027 sections → CORE-UA 402 can't satisfy Texts & Ideas" hallucination) while future-proofing the prompt so the FOSE phase does NOT have to tear it out. Commit `e092a6c`.
- [x] **Step 2 — sanity-run:** engine `tsc --noEmit` clean; `systemPrompt.test.ts` 28/28 green; committed (prompt-only).

**Two-layer distinction (binding hand-off for the FOSE phase):** requirement/DEGREE feasibility (catalog-determined via `poolMembersFor` = dept + level range; FOSE-independent; the frozen 7+1-axis validator) and SECTION-SCHEDULING feasibility (live FOSE sections; meeting-time conflicts / closed sections; near registration term; CAN drive a re-plan that cascades to later terms) are **cleanly separable in the code today**. The future FOSE section-conflict / re-planning mechanism (hard conflict → move a course's term forward; soft conflict → student rejects the live sections → multi-term re-plan with trade-off clarification) sits **ON TOP** of the existing `finalizeForwardSchedule` seam — every re-plan stays requirement-validated unchanged — and is **deferred to Phase 15/16** (the Decision #19 `swapHook` placement-feedback path is already stubbed `null` for exactly this). See the `fose-phase-section-feasibility-notes` memory for the full hand-off discussion.
...
**Also explicitly deferred — cross-school credit cap (NOT a P/F concern; logged 2026-06-19).** The in-NYU **cross-school credit cap** (e.g. CAS/Tandon "16 credits / 4 courses in other divisions" — `arts-science/academic-policies/_index.md:238`, `engineering/academic-policies/_index.md:156`) is a **residency/elective** cap, distinct from the P/F grading-mode cap — do NOT fold it into the 8th axis. It is currently **unmodeled**: the schema still reserves a `creditCapType: "non_home_school"` enum (`configSchema.ts`) but the `creditCaps[non_home_school]` entry was **deleted from every config in Step 8e part 2 (`5e33a44`)** — the per-student `outsideHomeCapUnits` is now read from the **DPR** (`dpr.cumulative.outsideHomeCapUnits`), not the config. Modeling + enforcing the *policy* cap is a separate future item.

**Do NOT run a `data/schools` "dead-field cleanup" (history checked 2026-06-19 — see the `data-schools-history` memory).** `data/schools` is a deliberately-curated, provenance-cited per-school **facts** dataset, and the real dead-field purge **already happened**: after the Phase-7E DPR-first pivot + the Step-8b rule-engine decommission, **Step 8e parts 1–3 (PRs #28–#30 — `2ee4a4c`/`5e33a44`/`3a5a051`)** deleted the orphaned rule-engine validators and every DPR-duplicate / inferred-default / orphaned field, establishing the rule *"configs record ONLY bulletin-confirmed facts."* The fields that remain are there **by design**: either consumed by `get_credit_caps` (5 fields) / `get_academic_standing` (3 fields) / the P/F axis + `pfEligibility` (`canElect`, `careerLimitType`/`Value`, `countsForMajor`/`Minor`/`GenEd`), **or** kept as cited **"asked-facts"** (`spsPolicy`, `courseSuffix`, the passFail policy detail) the agent can quote when asked. So most "unused-by-logic" fields are **NOT dead code** — `get_credit_caps.md` already documents *"No enforcement module … surfaces numbers; nothing downstream enforces."* A naive deletion pass would regress the asked-facts layer. The only genuinely-loose residue is small (`deansListThreshold`, `maxCourseRepeats`, `residency.finalCreditsInResidence`/`majorMinorResidencyPercent`, the not-yet-wired `passFail.perTermLimit`/`perTermUnit`); leave them as cited asked-facts (wiring per-term P/F is a *feature* decision — see **Open follow-ups** below — never a "cleanup"). ⚠ An earlier same-session field audit produced FALSE-DEAD verdicts (it marked `passFail.countsForMinor`/`countsForGenEd` dead, but `pfEligibility.ts:148-152` reads them live) — do not trust a "never-read-by-logic" scan as a delete-list.
...
## ⚠ Open follow-ups (post-implementation review — 2026-06-19; implementor please action)

These surfaced during the bulletin/P-F + `data/schools` review *after* Phases A/C/D/G/H/I had landed. They are not new scope — they reconcile the shipped code with this plan.

- **[P0] Per-term P/F tier is NOT implemented in the shipped 8th axis.** D-4 (above) lists `perTermLimit` over `perTermUnit` (semester / academic_year) as a **HARD** tier, but the committed axis (**C1 `0b4fc16` / C2 `72497ea`**) enforces only the **career** cap — `perTermLimit`/`perTermUnit` have **zero readers** (`configSchema.ts:70-71`, unread by `passFailLimitAxis.ts`). **Decide one:** (a) wire per-term into `checkPassFailLimits` (Stern/Gallatin are per-`academic_year`, most schools per-semester), or (b) consciously **descope** it and edit the D-4 per-term bullet to read DEFERRED. Right now the plan claims a tier the code doesn't enforce.
- **[P1] Cross-school P/F hedge for Nursing + Gallatin (messaging only — no axis change).** `pfEligibility` keys on the student's **home** school, which is correct for the dominant bulletin pattern (CAS/Stern/Tandon/etc. govern P/F by the home school even for non-home courses — Stern: *"including non-Stern"*). But **Nursing** and **Gallatin** govern P/F by the **offering** school. Add a "your home-school limit applies to all your P/F elections, but Nursing/Gallatin base eligibility on the offering school — verify with your adviser" hedge to the agent's P/F messaging for those two home schools (per the D-4 cross-school note). Soft.
- **[done / info — no action]** Tandon P/F prose corrected (the *elect-only* rule — a Tandon student can't convert a letter course to P/F, but inherently-P/F-only courses count as free electives; the `tandon.json` `passFail.note` + `slotActionMatrix.ts:144` copy were already accurate). The `data/schools` "dead-field cleanup" was **declined** (owner decision 2026-06-19 — the real purge was Step 8e; the dormant fields are retained cited facts for future use). Full field→mechanism map: `Docs/audits/2026-06-19-schoolconfig-field-enforcement-map.md`.
