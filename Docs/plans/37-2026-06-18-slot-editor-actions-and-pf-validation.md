...
**Decision (owner, 2026-06-18):** a scenario appears in the workspace as a compare-able tab ONLY when it is `valid-clean` or `valid-with-trade-offs`. An **invalid read-only what-if** is NOT rendered as a workspace tab — the agent explains WHY in chat (with the concrete consequence), as today. An **invalid committable proposed change** keeps its red review-card + "Override anyway" (`force`) surface (you can still adopt it as `student-preferred-invalid-draft`). Rationale: the workspace holds plans you could adopt or meaningfully compare; an invalid read-only exploration isn't one. **PRECONDITION:** only safe once the validity verdict is reliable — Phases J/K (the false-invalid empty-placeholder/binding bug) MUST land first, else this rule hides legitimately-valid explorations (e.g. the J-term).
