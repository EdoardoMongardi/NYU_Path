...
- **HARD (defines "valid" — the validator's axes):** requirement coverage by **bound** courses (PLAN-4) · prereqs in earlier terms · coreqs same/earlier term · terms-offered season match (PLAN-1) · per-term credit ceiling · per-term floor (F-1 ≥12 unless RCL / domestic part-time) · graduation ≤ goal target (PLAN-2) · aggregate floors (degree-total · major-credit · upper-level · residency incl. joint-major · school-core) (PLAN-4/DPR-6) · category caps (P/F · outside-home · online) · exclusions / no invalid double-count + repeat handling · taken=locked / IP=fixed-in-term · no time-conflict / not closed *(FOSE-light — the ONE item in this HARD list NOT implemented as a structural validator axis as of 2026-06-19: it is enforced at the **section-materialization layer** (`conflictDetection.ts` via `materialize_sections`) for the **immediate registration term only**, not by the 8-axis `graduationPathValidator`. Elevating it to force cross-term re-planning is the deferred FOSE re-plan phase — see §9).*
...
## 9. Layer ⑥ — FOSE live data (Phase 15 BUILT; cross-term re-plan + heavy axes deferred)

**BUILT (Phase 15 — verified against code 2026-06-19):** a live FOSE class-search client (`packages/engine/src/api/nyuClassSearch.ts` → `https://bulletins.nyu.edu/class-search/api/`), `search_availability`, meeting-time parsing (`meets` + `meetingTimes`, validated against 27 fixtures), availability classification (`foseAvailabilityGate.ts`), a 5-min TTL cache (`foseCache.ts`), near-term **time-conflict detection** (`conflictDetection.ts`, `MAX_COMBINATIONS=50`), a scheduling-preference filter (`applySchedulingPreferences.ts`, Decision #43), the 11-step `materialize.ts` orchestrator, and the two-step `materialize_sections` → `confirm_section_combination` flow (stages conflict-free CRN combinations for the immediate registration term; `confirm` attaches CRN/meeting/instructor metadata onto already-bound `specific_planned` slots in place — read-only on the course set, never re-validates).

**DEFERRED (the FOSE re-plan phase — the next phase):** the `swapHook` (Decision #19, `materializeSections.ts:198-201`) is stubbed `async () => null`, so a course with no conflict-free / open section currently just **drops** from the materialization rather than being swapped or moved. The owner's target — a **HARD** section conflict forcing a course to a **different term** (cascading forward) and a **SOFT** conflict (rejected sections / professors / recitation timing) re-planning current + future terms with multi-course trade-offs — is the deferred work, together with Albert auto-swap, waitlist number, campus/location, instruction mode, instructor preference, and recitation (LEC+RCT) pairing (none modeled today). The deferred **section-packing** sub-problem (sections × meeting-times × recitations) is where a pluggable CP-SAT/MILP backend would earn its keep, behind the same constraint-model interface — and **every such re-plan still flows through the frozen `finalizeForwardSchedule` (requirement-validated), so the section layer sits strictly on top of the 8-axis validator**.
...
- **Phase 1 · Foundation.** §5 normalizers + de-null-gated validator + one `SolverInput` builder. *Closes:* CAS-1/8, RC-2/3/4, HARD-2, PLAN-1/4/11, DPR-1…6. *Done when:* unified-builder golden tests pass on CAS + non-CAS + What-If; all 8 validator axes live. *(Plan-37 phases J–C2 added the 8th axis `passFailLimitsRespected` — the first owner-approved deliberate extension of the formerly-frozen 7-axis contract; see §7 and `forward-schedule.md` §7 for tiered semantics. The solver/search seam is unchanged.)*
